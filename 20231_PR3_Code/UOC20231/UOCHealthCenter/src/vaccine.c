...
void vaccine_init(tVaccine* vaccine, const char* name, int required, int days,int unitDoses) {
    assert(vaccine != NULL);
    assert(name != NULL);
...
void vaccine_cpy(tVaccine* destination, tVaccine source) {
    ...
    // Set the data
    vaccine_init(destination, source.name, source.required, source.days, source.unitDoses);
}


// Release vaccine lot data
void vaccineLot_init(tVaccineLot* lot, tVaccine* vaccine, const char* hcc, tDateTime timestamp, int doses) {
    assert(lot != NULL);
    assert(hcc != NULL);
    
    // Allocate memory for the cp
    lot->hcc = (char*) malloc(strlen(hcc) + 1);
    assert(lot->hcc != NULL);
...
void vaccineLot_cpy(tVaccineLot* destination, tVaccineLot source) {
    ...
    // Set the data
    vaccineLot_init(destination, source.vaccine, source.hcc, source.timestamp, source.doses);
}
...
void vaccineLot_parse(tVaccine* vaccine, tVaccineLot* lot, tCSVEntry entry) {
    ...
    unitDoses = csv_getAsInteger(entry, 7);
    ...
    vaccine_init(vaccine, buffer, required, days,unitDoses);
}
...
void vaccineLotData_add(tVaccineLotData* data, tVaccineLot lot) {
    ...
    idx = vaccineLotData_find(*data, lot.hcc, lot.vaccine->name, lot.timestamp);
...
void vaccineLotData_del(tVaccineLotData* data, const char* hcc, const char* vaccine, tDateTime timestamp, int doses) {
    int idx;
    int i;
    ...
    // Check if an entry with this data already exists
    idx = vaccineLotData_find(*data, hcc, vaccine, timestamp);
...
int vaccineLotData_find(tVaccineLotData data, const char* hcc, const char* vaccine, tDateTime timestamp) {
    int i;
