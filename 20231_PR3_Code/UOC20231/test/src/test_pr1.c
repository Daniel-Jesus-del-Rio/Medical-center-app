...
bool run_pr1_ex1(tTestSection* test_section, const char* input) {
    ...
    // Get the version
    version = api_version();
    if (strcmp(version, "UOC PP 20231") != 0) {
        failed = true;
        passed = false;
    }
...
bool run_pr1_ex2(tTestSection* test_section, const char* input) {
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX2_2", "Add a lot with invalid hcc");
    if (fail_all) {
        failed = true;
    } else {
        csv_initEntry(&entry);
        csv_parseEntry(&entry, "01/01/2022;13:45;08001;PFIZER;2;21;300;5", "VACCINE_LOT");
        error = api_addVaccineLot(&data, entry);
        if (error != E_INVALID_HEALTH_CENTER_CODE) {
            failed = true;
            passed = false;
            fail_all = true;
        }
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX2_3", "Add a valid lot");
    if (fail_all) {
        failed = true;
    } else {
        csv_initEntry(&entry);
        csv_parseEntry(&entry, "01/01/2022;13:45;CA08001;PFIZER;2;21;300;5", "VACCINE_LOT");
        error = api_addVaccineLot(&data, entry);
        if (error != E_SUCCESS) {
            failed = true;
            passed = false;
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX2_4", "Check the number of vaccines");
    if (fail_all) {
        failed = true;
    } else {
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX2_5", "Check the number of vaccine lots");
    if (fail_all) {
        failed = true;
    } else {        
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX2_6", "Add a lot with invalid type");    
    if (fail_all) {
        failed = true;
    } else {
        csv_initEntry(&entry);
        csv_parseEntry(&entry, "01/01/2022;13:45;CA08001;PFIZER;2;21;300", "VACCINE_LOT_DATA");
        error = api_addVaccineLot(&data, entry);
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX2_7", "Add a lot with invalid fields");    
    if (fail_all) {
        failed = true;
    } else {
        csv_initEntry(&entry);
        csv_parseEntry(&entry, "01/01/2022;13:45;CA08001;PFIZER;2;21;300;5;extra_field", "VACCINE_LOT");
        error = api_addVaccineLot(&data, entry);
...
bool run_pr1_ex3(tTestSection* test_section, const char* input) {
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX3_3", "Request vaccine lot with invalid hcc");
    if (fail_all) {
        failed = true;
    } else {        
        csv_initEntry(&entry);
        csv_initEntry(&refEntry);
        csv_parseEntry(&refEntry, "01/01/2022;13:45;CA08001;PFIZER;2;21;350", "VACCINE_LOT");
        ...
        if (error != E_INVALID_HEALTH_CENTER_CODE) {
            ...
            error = api_getVaccineLot(data, "cA08001", "PFIZER", timestamp, &entry);
            if (error != E_INVALID_HEALTH_CENTER_CODE) {
                failed = true;
                passed = false; 
                fail_all = true;           
            } else {
                error = api_getVaccineLot(data, "CA080O1", "PFIZER", timestamp, &entry);
                if (error != E_INVALID_HEALTH_CENTER_CODE) {
                    failed = true;
                    passed = false; 
                    fail_all = true;           
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX3_4", "Request a valid vaccine lot");
    if (fail_all) {
        failed = true;
    } else {        
        csv_initEntry(&entry);
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX3_5", "Request a missing vaccine lot");
    if (fail_all) {
        failed = true;
    } else {
        csv_initEntry(&entry);
        dateTime_parse(&timestamp, "01/01/2022", "13:45");
        error = api_getVaccineLot(data, "CA10001", "NO_VACC", timestamp, &entry);
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX3_6", "Get registered vaccines");
    if (fail_all) {
        failed = true;
    } else {
        csv_init(&report);
    ...
    /////////////////////////////
    failed = false;
    start_test(test_section, "PR1_EX3_7", "Get registered vaccine lots");
    if (fail_all) {
        failed = true;
    } else {
        csv_init(&report);
        csv_init(&refReport);
        csv_addStrEntry(&refReport, "01/01/2022;13:45;CA08001;PFIZER;2;21;350", "VACCINE_LOT");
        csv_addStrEntry(&refReport, "01/01/2022;13:45;CA08500;PFIZER;2;21;300", "VACCINE_LOT");
        csv_addStrEntry(&refReport, "02/01/2022;18:00;CA08500;MODERNA;1;0;100", "VACCINE_LOT");
        csv_addStrEntry(&refReport, "02/01/2022;15:45;CA08001;MODERNA;1;0;100", "VACCINE_LOT");
        csv_addStrEntry(&refReport, "03/01/2022;13:45;CA08500;PFIZER;2;21;70", "VACCINE_LOT");               
